...
#            'block_hash': self.__str(source['blockHash']),
    ...
    def get_transactions_by_address(self,
        ...
        self._params['address'] = address
        self._params['startblock'] = str(start_block)
        self._params['endblock'] = str(end_block)
        self._params['page'] = str(page)
        self._params['offset'] = str(limit)
    ...
    def __event(self, source: dict):
        """Repack the __event dict"""
        return {
            'timestamp': self.__int(source['timeStamp']),
            'block_number': self.__int(source['blockNumber']),

            'address': self.__str(source['address']),
            'topics': source['topics'],
            'data': self.__str(source['data']),

            'gas_price': self.__int(source['gasPrice']),
            'gas_used': self.__int(source['gasUsed']),
            'logIndex': self.__str(source['logIndex']),
            'transactionHash': self.__str(source['transactionHash']),
            'transactionIndex': self.__str(source['transactionIndex']),
        }
    ...
    def get_contract_events(self,
        ...
        self._params['module'] = 'logs'
        self._params['action'] = 'getLogs'
        self._params['fromBlock'] = str(start_block)
        self._params['toBlock'] = str(end_block)
        self._params['address'] = contract_address
        self._params['topic0'] = topic0

        rs = self.__req()

        events = list()
        for event in rs:
            events.append(self.__event(event))
